...
def load_data_set(filename):
    try:
        with open(filename, newline='') as test:
            return list(reader(test, delimiter=','))
...
def knn(training_set, test_set, k):
    ...
    try:
        for test_instance in test_set:
            ...
            # Display prediction
            if value/float(k)==0.5:
                print("Can Be Both Of " ,end=' ')
                print(classes,end='  ')
            else:
                print('The Predicted class is : ' + classes[index])
                print('Number of votes : ' + str(value) + ' From ' + str(k))
